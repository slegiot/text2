...
# ---------- mmcv-free replacements ----------
class ConvModule(nn.Module):
    """Drop-in replacement for mmcv.cnn.ConvModule (Conv + optional BN + optional Act).
    Uses the same attribute names as mmcv so pretrained state_dicts load correctly."""
    def __init__(self, in_channels, out_channels, kernel_size,
                 stride=1, padding=0, norm_cfg=None, act_cfg=None, **kwargs):
        super().__init__()
...
def _load_checkpoint(model, url, strict=True):
    """Replacement for mmcv.runner.load_checkpoint — loads weights from a URL."""
    state_dict = torch.hub.load_state_dict_from_url(url, map_location='cpu', progress=True)
    if 'state_dict' in state_dict:
        state_dict = state_dict['state_dict']
    model.load_state_dict(state_dict, strict=strict)
# ---------- end mmcv-free replacements ----------
    ...
    def __init__(
        ...
        if use_pretrain:
            if isinstance(pretrained, str):
                ...
                _load_checkpoint(self, pretrained, strict=True)
...
def flow_warp(x,
    ...
    """
    # MPS doesn't support 'border' padding mode in grid_sample
